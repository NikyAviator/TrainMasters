...
export class FetchHelper {
  static async findStations(station) {
    return [await (await fetch(`/api/routesInfo/${station}`)).json()]
      .flat()
  ...
  static async findOneBookings(id) {
    return [await (await fetch(`/api/bookings/${id}`)).json()]
      .flat()
      .map((x) => (x ? new this(x) : null));
  }
  static async seatsCarriage() {
    return [await (await fetch(`/api/seatsCarriage`)).json()]
      .flat()
      .map((x) => (x ? new this(x) : null));
  }
